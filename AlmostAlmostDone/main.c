...
int main(void) {
	...
	//Var tionde interrupt (var sekund) så körs detta
	//Även så adderas en global konstant som har koll på sekunder
	//Vi sparar alla temps i en variabel
	//och delar den variabeln på antal sekunder för att få avgtemp
	for(;;) {
		...
		do {
			...
			medeltemperatur = avgtemp / sekundcount;
			...
			*t++ = 0;
			...
			if(switchez == 4)
				{
					s = fixed_to_string(medeltemperatur, buf);
